...
<h6 class="mb-3 mt-4"><?php echo t("Mobile API URL")?></h6>    
  ...
  <div class="form-label-group mt-2">    
   <?php echo $form->textArea($model,'tableside_jwt_token',array(
     'class'=>"form-control form-control-text",     
     'placeholder'=>t("Contact Content"),
     'disabled'=>true,     
   )); ?>      
   <?php echo $form->error($model,'tableside_jwt_token'); ?>
   <a class="btn btn-link" href="<?php echo $delete_keys?>"><?php echo t("Delete Keys")?></a>
</div>

<?php else :?>
 
 <div class="form-label-group">    
   <?php echo $form->hiddenField($model,'tableside_jwt_token',array(
     'class'=>"form-control form-control-text",
     'placeholder'=>$form->label($model,'tableside_jwt_token'),     
   )); ?>       
   <?php echo $form->error($model,'tableside_jwt_token'); ?>
</div>
