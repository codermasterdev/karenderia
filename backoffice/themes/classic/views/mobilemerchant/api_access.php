...
<h6 class="mb-3 mt-4"><?php echo t("Mobile API URL")?></h6>    
  ...
  <div class="form-label-group mt-2">    
   <?php echo $form->textArea($model,'merchant_jwt_token',array(
     'class'=>"form-control form-control-text",     
     'placeholder'=>t("Contact Content"),
     'disabled'=>true,     
   )); ?>      
   <?php echo $form->error($model,'merchant_jwt_token'); ?>
   <a class="btn btn-link" href="<?php echo Yii::app()->createUrl("/mobilemerchant/delete_apikeys")?>"><?php echo t("Delete Keys")?></a>
</div>

<?php else :?>
 
 <div class="form-label-group">    
   <?php echo $form->hiddenField($model,'merchant_jwt_token',array(
     'class'=>"form-control form-control-text",
     'placeholder'=>$form->label($model,'merchant_jwt_token'),     
   )); ?>   
   <?php    
    //echo $form->labelEx($model,'merchant_jwt_token'); ?>
   <?php echo $form->error($model,'merchant_jwt_token'); ?>
</div>
